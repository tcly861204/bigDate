(function (window) {
  ...
  ({
    init:function(){
        ...
        this.searchDate = {
            ...
            operate_type: '',
            dateType: '出团日期',
            startdate: this.setQueryTime(1),
            enddate: this.getTimeString(this.nowTime)
        };
        this.faceListSearch = $('#Js_faceList');
        this.faceListStart = $('#Js_s_faceList');
        this._init();
    },
    saleListTpl:function (obj) {
    ...
    getSaleFaceList : function (skey, _node, from) {
      if (skey === void 0) { skey = ''; }
      var that = this;
      this.showLoading();
      $.post(this.getDateUrl.sale, { saleName: skey }, function (res) {
          ...
          if (res.saleWhole.length) {
              var str_1 = '';
              if(!that.allFaceList){
                  that.allFaceList = res.saleWhole;
              }
  ...
  resetSaleFace : function(){
    var that = this;
    var str = '';
    $.each(this.allFaceList, function (index, item) {
      str += that.sfaceListTpl(item);
    });
    this.faceListSearch.html(str);
  },
  //筛选
  ...
  setQueryTime : function (m) {
      ...
      this.searchDate.startdate = this.getTimeString(start);
      this.searchDate.enddate = this.getTimeString(this.nowTime);
      this.searchStartDate.val(this.searchDate.startdate);
      this.searchEndDate.val(this.searchDate.enddate);
  },
  //外层翻页
  ...
  loadDatePages : function (pagestr, fn) {
      var that = this;
      this.showLoading();
  ...
  //获取第一页数据
  getSaleInfos : function () {
      var that = this;
      this.showLoading();
      $.post(this.getDateUrl.itemSale, that.searchDate, function (data) {
          that.hideLoading();
          that.searchDate.saleName = data.saleSingles.saleName;
          that.searchDate.saleID = data.saleSingles.saleID;
  ...
  salePropCvs : function (id, num, color) {
      ...
      function run() {
          ...
          ctx.stroke();
          if (ctx.Num - num > 0 && ctx.Num - num <= 2) {
  ...
  saleTypeBit : function () {
      ...
      var iWd = 0;
      if (len > 6) {
          this.saleTypeBitNodes.removeClass('min');
          ...
          this.saleTypeBitNodes.addClass('min min-'+len);
  ...
  unitPrice : function () {
      ...
      cvsNode.show();
      if (data.length < 1) {
          $('#Js_page_noDate_5').show();
          cvsNode.hide();
          return false;
      }
      else {
          $('#Js_page_noDate_5').hide();
      }
      ...
      Len = Math.floor(cvsNode.parent().parent().height() / _LH) > Len ? Len : Math.floor(cvsNode.parent().height() / _LH);
